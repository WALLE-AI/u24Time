...
"""
    ...
    async def generate_report_stream(self, simulation_id: str, graph_id: str, query: str):
        """
        生成 Markdown 格式预测报告并流式返回
        """
        logger.info(f"PredictionReportAgent: 正在为仿真 {simulation_id} (图谱 {graph_id}) 处理提问 (流式): {query}")
...
"""
        ...
        try:
            ...
            logger.error(f"PredictionReportAgent: 报告流式生成失败 - {e}")
            yield f"\n\n推演分析生成受到外部网络因素影响，暂时未能完成。\n\n异常反馈：{e}"
